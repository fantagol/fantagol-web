-- ============================================================================
-- FANTAGOL
-- Migration 269: Standings Points + Identity Projection Repair
--
-- Purpose:
--   Repair the semantic contract consumed by StandingsPreviewBuilder.
--
-- Root cause:
--   points_preview.members is the compact scoring projection and exposes
--   pure_points, exact_count, bonus_count and malus_count.
--   It does not expose round_points or display_name.
--
--   The previous StandingsPreviewBuilder consumed:
--     m.display_name
--     m.round_points
--
--   producing:
--     display_name = NULL for every member
--     round_points = 0 for every member
--
-- Certified production forensic:
--   La Grande Bellezza 2.0 / Round 1
--   canonical scoring total       = 601.00
--   corrupted certified total     =   0.00
--   lost points                   = 601.00
--   point-corrupted members       = 19/20
--   identity-corrupted members    = 20/20
--
-- Repair:
--   - pure_round_points comes from points_preview.members.pure_points;
--   - display_name comes from league_members bound by exact member + league;
--   - active membership is required;
--   - all other StandingsPreviewBuilder behavior is preserved byte-for-byte.
--
-- Safety:
--   - no historical certification mutation;
--   - no historical ledger mutation;
--   - no runtime result mutation;
--   - repaired lineage must be rebuilt and certified through normal authority.
-- ============================================================================
