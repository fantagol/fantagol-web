-- ============================================================================
-- FANTAGOL MIGRATION 313
-- LEAGUE GOVERNANCE ADMIN ACTIVITY LOCK REPAIR
--
-- Root cause:
-- evaluate_league_admin_activity_rpc selected the ranking-based successor
-- through a LEFT JOIN LATERAL aggregate query and applied FOR UPDATE to that
-- aggregate-containing SELECT. PostgreSQL rejects that statement with 0A000:
-- FOR UPDATE is not allowed with aggregate functions.
--
-- Repair:
-- 1. preserve the exact certified ranking and tie-break order;
-- 2. select only the winning league_member UUID without a row lock;
-- 3. lock and materialize exactly that league_members row separately;
-- 4. preserve Admin and Vice locking semantics.
--
-- Source authority:
-- 268_league_governance_conflict_target_hotfix.sql
-- ============================================================================
    ...
    if v_resulting_missed = 1 then
      ...
      if v_successor.id is not null then
        ...
        if v_has_official_ranking then
          ...
          if v_r19_successor_id is not null then
            select lm.*
            into v_successor
            from public.league_members lm
            where lm.id = v_r19_successor_id
            for update;
          else
            v_successor := null;
          end if;
