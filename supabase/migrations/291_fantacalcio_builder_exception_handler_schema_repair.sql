-- MIGRATION 291
-- FANTAGOL R52-R5-R9
-- FANTACALCIO LIVE ROW CONTRIBUTION MATERIALIZATION
-- ADDITIVE PAYLOAD EXTENSION ONLY
-- SOURCE AUTHORITY: build_fantacalcio_preview_simulation_rpc
-- PRE-LIVE/UI BEHAVIOR UNCHANGED
...
begin
  ...
  values (
    v_simulation.id, v_round.id, v_run.id, 'RoundSimulationPreviewReady',
    jsonb_build_object(
...
'R52-R5-R9 / Migration 291 exception-handler compatibility repair: additive Fantacalcio preview extension. Materializes authoritative per-match contributions inside each fixture side. Contributions are derived from the exact canonical scoring expression and reconcile to side points.';
