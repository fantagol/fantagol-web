-- =============================================================================
-- FANTAGOL
-- Migration: 134_commercial_secret_reference_resolution_authorization_handoff_lifecycle_certification.sql
-- Milestone: Commercial Secret Reference Resolution Authorization Handoff
--
-- Purpose:
--   Transactionally certify migration 133 through the complete passive chain:
--
...
begin
  ...
  if to_regclass('public.commercial_secret_reference_handoff_policies') is null
    ...
    raise exception 'MIGRATION_134_REQUIRES_MIGRATION_133_OBJECTS';
  end if;
  ...
  if to_regprocedure(
    ...
    raise exception 'MIGRATION_134_REQUIRES_MIGRATION_133_FUNCTIONS';
  end if;
  ...
  if not (
    ...
    raise exception 'MIGRATION_134_HANDOFF_CAPABILITY_MATRIX_DISABLED';
  end if;
  ...
  if v_handoff_policy.automatic_dispatch_enabled
    ...
    raise exception 'MIGRATION_134_UNSAFE_HANDOFF_POLICY';
  end if;
  ...
  v_backend_code := 'migration_134_backend_' || v_suffix;
  v_backend_name := 'Migration 134 Passive Backend ' || upper(v_suffix);
  v_namespace := 'migration_134:opaque:' || v_suffix;
  v_binding_key := 'migration_134:binding:' || v_suffix;
  ...
  v_gateway_request :=
    public.enqueue_commercial_secret_reference_gateway_request(
      'migration_134:gateway_request:'||v_suffix,
      'migration_134:gateway_idempotency:'||v_suffix,
      null,
      null,
      'production',
      v_namespace,
  ...
  if v_gateway_decision.decision_status<>'accepted'
    ...
    raise exception 'MIGRATION_134_GATEWAY_PREREQUISITE_FAILED';
  end if;
  ...
  v_authorization_request :=
    public.enqueue_commercial_secret_reference_authorization_request(
      'migration_134:authorization:'||v_suffix,
      'migration_134:authorization_idempotency:'||v_suffix,
      v_gateway_request.id,
  ...
  if v_authorization_decision.decision_status<>'authorized'
    ...
    raise exception 'MIGRATION_134_AUTHORIZATION_PREREQUISITE_FAILED';
  end if;
  ...
  v_handoff_request :=
    public.enqueue_commercial_secret_reference_handoff_request(
      'migration_134:handoff:'||v_suffix,
      'migration_134:handoff_idempotency:'||v_suffix,
      v_authorization_request.id,
  ...
  if v_handoff_replay.id<>v_handoff_request.id then
    raise exception 'MIGRATION_134_IDEMPOTENT_REPLAY_CREATED_DUPLICATE';
  end if;
  ...
  begin
    ...
    raise exception 'MIGRATION_134_IDEMPOTENCY_CONFLICT_NOT_ENFORCED';
  exception
    when others then
  ...
  if v_handoff_decision.decision_status<>'accepted'
    ...
    raise exception 'MIGRATION_134_HANDOFF_ACCEPTED_PATH_FAILED';
  end if;
  ...
  if v_handoff_decision.dispatch_allowed
    ...
    raise exception 'MIGRATION_134_HANDOFF_DECISION_NOT_PASSIVE';
  end if;
  ...
  if v_handoff_manifest.manifest_status<>'held'
    ...
    raise exception 'MIGRATION_134_HANDOFF_MANIFEST_FAILED';
  end if;
  ...
  if v_handoff_attempt.attempt_type<>'SECRET_RESOLUTION'
    ...
    raise exception 'MIGRATION_134_BLOCKED_ATTEMPT_FAILED';
  end if;
  ...
  if v_read.request_status<>'accepted'
    ...
    raise exception
      'MIGRATION_134_READ_MODEL_FAILED evaluations=%, attempts=%, receipts=%, events=%, replay=%',
  ...
  if v_backend_count<>1
    ...
    raise exception
      'MIGRATION_134_COUNT_ASSERTION_FAILED backend=%, version=%, capabilities=%, binding=%, gateway_request=%, gateway_decision=%, authorization_request=%, authorization_evaluations=%, authorization_decision=%, handoff_request=%, handoff_idempotency=%, handoff_evaluations=%, handoff_decision=%, handoff_manifest=%, handoff_attempt=%, handoff_receipts=%, handoff_events=%, replay=%',
      v_backend_count,
      v_version_count,
      v_capability_count,
      v_binding_count,
  ...
  begin
    ...
    raise exception 'MIGRATION_134_POLICY_GUARD_NOT_ENFORCED';
  exception
    when others then
  ...
  begin
    ...
    raise exception 'MIGRATION_134_EVALUATION_GUARD_NOT_ENFORCED';
  exception
    when others then
      if sqlerrm='COMMERCIAL_SECRET_REFERENCE_HANDOFF_IMMUTABLE' then
  ...
  begin
    ...
    raise exception 'MIGRATION_134_DECISION_GUARD_NOT_ENFORCED';
  exception
    when others then
      if sqlerrm='COMMERCIAL_SECRET_REFERENCE_HANDOFF_IMMUTABLE' then
  ...
  begin
    ...
    raise exception 'MIGRATION_134_MANIFEST_GUARD_NOT_ENFORCED';
  exception
    when others then
      if sqlerrm='COMMERCIAL_SECRET_REFERENCE_HANDOFF_IMMUTABLE' then
  ...
  begin
    ...
    raise exception 'MIGRATION_134_ATTEMPT_GUARD_NOT_ENFORCED';
  exception
    when others then
  ...
  begin
    ...
    raise exception 'MIGRATION_134_RECEIPT_GUARD_NOT_ENFORCED';
  exception
    when others then
      if sqlerrm='COMMERCIAL_SECRET_REFERENCE_HANDOFF_APPEND_ONLY' then
  ...
  begin
    ...
    raise exception 'MIGRATION_134_EVENT_GUARD_NOT_ENFORCED';
  exception
    when others then
      if sqlerrm='COMMERCIAL_SECRET_REFERENCE_HANDOFF_APPEND_ONLY' then
        v_event_guard:=true;
  ...
  begin
    insert into public.commercial_secret_reference_handoff_requests(
      ...
      'migration_134:plaintext:'||v_suffix,
      'migration_134:plaintext_idempotency:'||v_suffix,
      v_handoff_policy.id,
      v_authorization_request.id,
      v_authorization_decision.id,
      v_gateway_request.id,
    ...
    raise exception 'MIGRATION_134_PLAINTEXT_GUARD_NOT_ENFORCED';
  exception
  ...
  begin
    insert into public.commercial_secret_reference_handoff_requests(
      ...
      'migration_134:endpoint:'||v_suffix,
      'migration_134:endpoint_idempotency:'||v_suffix,
      v_handoff_policy.id,
      v_authorization_request.id,
      v_authorization_decision.id,
      v_gateway_request.id,
    ...
    raise exception 'MIGRATION_134_ENDPOINT_GUARD_NOT_ENFORCED';
  exception
    when check_violation then
  ...
  begin
    insert into public.commercial_secret_reference_handoff_requests(
      ...
      'migration_134:executable_probe:'||v_suffix,
      'migration_134:executable_probe_idempotency:'||v_suffix,
      v_handoff_policy.id,
      v_authorization_request.id,
      v_authorization_decision.id,
      v_gateway_request.id,
    ...
    insert into public.commercial_secret_reference_handoff_manifests(
      ...
      'migration_134:executable:'||v_suffix,
      v_probe_handoff_request_id,
    ...
    raise exception 'MIGRATION_134_EXECUTABLE_MANIFEST_GUARD_NOT_ENFORCED';
  exception
    when check_violation then
  ...
  if not (
    ...
    raise exception 'MIGRATION_134_GUARD_MATRIX_FAILED';
  end if;
  ...
  if v_purchase_after<>v_purchase_before
    ...
    raise exception 'MIGRATION_134_ECONOMIC_NON_MUTATION_FAILED';
  end if;
...
\echo ''
\echo 'MIGRATION_134_TRANSACTION_ROLLED_BACK'
\echo 'No temporary backend, capability, binding, gateway object, authorization object, handoff object, policy change, purchase, checkout, wallet, ledger or outbox mutation was persisted.'
