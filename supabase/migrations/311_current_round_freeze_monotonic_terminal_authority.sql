-- FANTAGOL MIGRATION 311
-- CURRENT-ROUND 12H FREEZE: MONOTONIC FIRST TERMINAL AUTHORITY
--
-- A historical/admin recertification may replace the active official certification,
-- publication, profile fanout and reward lineage, but it must not restart the UX
-- post-round freeze clock once the round already reached a complete terminal state.
--
-- Preserve M294 semantics:
--   * freeze only after publication + profile fanout + production-required rewards
--     are terminal;
--   * 12h duration;
--   * freeze priority remains above next predictions_open round.
--
-- Change only the temporal authority:
--   * derive terminal completion for every committed official/superseded certification;
--   * preserve superseded publications as historical published evidence;
--   * choose MIN(terminal_at) per league_round;
--   * require a current active official certification as the round authority gate.
--
-- No schedule/scoring/recovery/reward/standings mutation.
...
begin
  ...
  certification_history as (
    select
      ...
      rc.committed_at
    from public.round_certifications rc
    join active_certification ac
      on ac.league_round_id=rc.league_round_id
    where rc.committed_at is not null
      and rc.status in ('official','superseded')
  ),
  ...
  publication_terminal as (
    select
      ...
      ch.league_round_id,
      count(*) filter (
        where rsp.status in ('published','superseded')
          and rsp.published_at is not null
          and rsp.metadata->>'certification_id'=ch.certification_id::text
    ...
    left join public.round_simulation_publications rsp
      on rsp.league_round_id=ch.league_round_id
     and rsp.metadata->>'certification_id'=ch.certification_id::text
    group by ch.certification_id,ch.league_round_id
  ),
  profile_terminal as (
    select
      ch.certification_id,
    ...
    join active_member_expectation ame
      on ame.league_round_id=ch.league_round_id
    left join public.live_runtime_workflows w
     ...
     and w.metadata->>'league_round_id'=ch.league_round_id::text
     and w.metadata->>'round_certification_id'=ch.certification_id::text
    ...
    group by ch.certification_id,ch.league_round_id,ame.expected_profile_workflows
  ),
  certified_results as (
    ...
    join public.prediction_score_runtime_results psrr
      on psrr.calculation_run_id=ch.source_run_id
     and psrr.league_round_id=ch.league_round_id
  ...
  reward_terminal as (
    ...
    left join production_required_rewards rrq
      on rrq.certification_id=ch.certification_id
     and rrq.league_round_id=ch.league_round_id
    left join public.loyalty_reward_events lre
      on lre.league_round_id=ch.league_round_id
  ...
  certification_terminal as (
    ...
    join publication_terminal pt
      on pt.certification_id=ch.certification_id
     and pt.league_round_id=ch.league_round_id
    join profile_terminal pr
      on pr.certification_id=ch.certification_id
     and pr.league_round_id=ch.league_round_id
    join reward_terminal rt
      on rt.certification_id=ch.certification_id
     and rt.league_round_id=ch.league_round_id
  ...
  terminal_round as (
    select
      ac.league_round_id,
      min(ct.terminal_at) as freeze_anchor_at
    from active_certification ac
    join certification_terminal ct
      on ct.league_round_id=ac.league_round_id
    where ct.terminal_at is not null
    group by ac.league_round_id
  ),
  freeze_round as (
