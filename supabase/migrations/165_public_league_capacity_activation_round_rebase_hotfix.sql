-- ============================================================================
-- 165_public_league_capacity_activation_round_rebase_hotfix.sql
--
-- FantaGol Milestone 12.9.5.12.2
--
-- Hotfixes the internal activation core installed by migration 164:
--   * skips league_round renumbering when the canonical 1..N layout is intact;
--   * uses a dynamic collision-safe parking offset when rebasing is required;
--   * reports only truly inserted league_round rows;
--   * preserves the RPC signatures, grants and external behavior.
-- ============================================================================
...
begin
  select pg_get_functiondef(
    'public.activate_league_roster_internal(uuid,uuid,uuid,uuid,text,boolean,text)'::regprocedure
  )
  into v_definition;

  if position('v_requires_round_rebase' in v_definition) = 0 then
    raise exception 'CONDITIONAL_ROUND_REBASE_NOT_INSTALLED';
  end if;

  if position('max(lr.league_round_number), 0) + 1000' in v_definition) = 0 then
    raise exception 'DYNAMIC_ROUND_OFFSET_NOT_INSTALLED';
  end if;
