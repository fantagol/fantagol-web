-- =====================================================================
-- FANTAGOL MIGRATION 286
-- LIVE REFRESH future-skew threshold-to-observed hardening
-- =====================================================================
...
declare
  ...
  v_provider_reported_updated_at timestamptz;
  v_provider_effective_updated_at timestamptz;
...
begin
  ...
  begin
    v_provider_reported_updated_at :=
  ...
  if v_provider_reported_updated_at is null then
    raise exception using
      errcode = 'P0001',
  ...
  v_provider_effective_updated_at :=
    case
      when v_provider_reported_updated_at > v_observed_at + interval '5 minutes'
        then v_observed_at
      else v_provider_reported_updated_at
    end;
  ...
  if v_match.provider_updated_at is not null
     and v_provider_effective_updated_at < v_match.provider_updated_at then
    return query
    select
      v_match.id,
      false,
  ...
  if v_provider_effective_updated_at is not distinct from v_match.provider_updated_at
  ...
  set
    ...
    minute = v_minute,
    period = v_period,
    provider_updated_at = v_provider_effective_updated_at,
    finalised_at = case
