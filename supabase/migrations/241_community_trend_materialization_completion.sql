-- FantaGol R39 Control Room Trend Semantics
-- Migration 241 - Community trend materialization completion
-- No schema changes.
-- Existing hashing, quality, registry and idempotency semantics preserved.
...
begin
    ...
    loop
        ...
        values (
            ...
            v_exact.rank = 1,
            case
                when v_previous_snapshot_id is null then null
                else (
                    select v_exact.prediction_percent - previous_exact.prediction_percent
                    from public.community_exact_distributions previous_exact
                    where previous_exact.community_snapshot_id = v_previous_snapshot_id
                      and previous_exact.match_id = v_exact.match_id
                      and previous_exact.home_prediction = v_exact.home_prediction
                      and previous_exact.away_prediction = v_exact.away_prediction
                    limit 1
                )
            end
        );
    ...
    if v_previous_snapshot_id is not null then
        insert into public.community_trends (
            fantagol_round_id, match_id, from_snapshot_id, to_snapshot_id,
            window_code, metric_code, outcome_code,
            from_value, to_value, delta_value, delta_percent,
            direction, velocity, stability, momentum, late_shift
        )
        select
            p_fantagol_round_id,
            ...
            p_phase = 'lock' and v_round.lock_at - clock_timestamp() <= interval '6 hours'
        ...
        cross join lateral (
            values
                ('sign_share', '1', previous_match.home_pick_percent, current_match.home_pick_percent),
                ('sign_share', 'X', previous_match.draw_pick_percent, current_match.draw_pick_percent),
                ('sign_share', '2', previous_match.away_pick_percent, current_match.away_pick_percent),
                ('over_under_share', 'OVER_2_5', previous_match.over_2_5_percent, current_match.over_2_5_percent),
                ('over_under_share', 'UNDER_2_5', previous_match.under_2_5_percent, current_match.under_2_5_percent),
                ('goal_no_goal_share', 'GOAL', previous_match.goal_percent, current_match.goal_percent),
                ('goal_no_goal_share', 'NO_GOAL', previous_match.no_goal_percent, current_match.no_goal_percent),
                ('consensus_index', null, previous_match.consensus_index, current_match.consensus_index),
                ('confidence_index', null, previous_match.confidence_index, current_match.confidence_index),
                ('chaos_index', null, previous_match.chaos_index, current_match.chaos_index)
        ...
        select
            p_fantagol_round_id,
            coalesce(previous_exact.match_id, current_exact.match_id),
            v_previous_snapshot_id,
            v_snapshot_id,
            'snapshot_to_snapshot',
            'exact_share',
            coalesce(previous_exact.home_prediction, current_exact.home_prediction)::text
                || '-' ||
            coalesce(previous_exact.away_prediction, current_exact.away_prediction)::text,
            coalesce(previous_exact.prediction_percent, 0),
            coalesce(current_exact.prediction_percent, 0),
            coalesce(current_exact.prediction_percent, 0) - coalesce(previous_exact.prediction_percent, 0),
            case
                when coalesce(previous_exact.prediction_percent, 0) = 0 then null
                else ((coalesce(current_exact.prediction_percent, 0) - previous_exact.prediction_percent)
                    / abs(previous_exact.prediction_percent)) * 100
            end,
            case
                when abs(coalesce(current_exact.prediction_percent, 0) - coalesce(previous_exact.prediction_percent, 0)) < 0.5 then 'stable'
                when coalesce(current_exact.prediction_percent, 0) > coalesce(previous_exact.prediction_percent, 0) then 'rising'
                else 'falling'
            end,
            abs(coalesce(current_exact.prediction_percent, 0) - coalesce(previous_exact.prediction_percent, 0)),
            greatest(0, 100 - abs(coalesce(current_exact.prediction_percent, 0) - coalesce(previous_exact.prediction_percent, 0))),
            case
                when abs(coalesce(current_exact.prediction_percent, 0) - coalesce(previous_exact.prediction_percent, 0)) < 0.5 then 'stable'
                when coalesce(current_exact.prediction_percent, 0) > coalesce(previous_exact.prediction_percent, 0) then 'rising'
                else 'falling'
            ...
            p_phase = 'lock' and v_round.lock_at - clock_timestamp() <= interval '6 hours'
        from (
            select * from public.community_exact_distributions
            where community_snapshot_id = v_previous_snapshot_id
        ) previous_exact
        full outer join (
            select * from public.community_exact_distributions
            where community_snapshot_id = v_snapshot_id
        ) current_exact
          on current_exact.match_id = previous_exact.match_id
         and current_exact.home_prediction = previous_exact.home_prediction
         and current_exact.away_prediction = previous_exact.away_prediction;
    end if;
