-- ============================================================================
-- FANTAGOL
-- Migration 308: Standings cumulative stats projection
--
-- Authority:
--   - league_ranking_ledger.active = true
--   - standings_delta.round_stats = immutable certified per-round stat delta
--   - historical rounds are strictly league_round_number < current round
--
-- Contract:
--   - preserve round_stats as CURRENT ROUND semantics
--   - add cumulative_stats for Fantacalcio and One-to-One
--   - do not change points, ranking, tiebreakers, ledger writer or official reader
--   - Punti Puri remains untouched
-- ============================================================================

begin;
create or replace function public.apply_cumulative_standings_stats_internal(
  p_league_round_id uuid,
  p_standings_preview jsonb
)
returns jsonb
language plpgsql
stable
...
declare
  ...
  v_result jsonb := p_standings_preview;
  v_mode text;
  v_ranking jsonb;
begin
  ...
  if p_standings_preview is null
     or jsonb_typeof(p_standings_preview) <> 'object' then
    raise exception using
      errcode = 'P0001',
      message = 'STANDINGS_PREVIEW_REQUIRED';
  end if;
  ...
  foreach v_mode in array array['fantacalcio','one_to_one']::text[]
  loop
    if jsonb_typeof(v_result #> array['modes',v_mode,'ranking']) <> 'array' then
      continue;
    end if;
    ...
    select coalesce(
      jsonb_agg(
        case
          when v_mode = 'fantacalcio' then
            r.member_row || jsonb_build_object(
              'cumulative_stats',
              jsonb_build_object(
                'wins',
                  coalesce(h.wins,0)
                  + coalesce((r.member_row #>> '{round_stats,wins}')::integer,0),
                'draws',
                  coalesce(h.draws,0)
                  + coalesce((r.member_row #>> '{round_stats,draws}')::integer,0),
                'losses',
                  coalesce(h.losses,0)
                  + coalesce((r.member_row #>> '{round_stats,losses}')::integer,0),
                'goals_for',
                  coalesce(h.goals_for,0)
                  + coalesce((r.member_row #>> '{round_stats,goals_for}')::integer,0),
                'goals_against',
                  coalesce(h.goals_against,0)
                  + coalesce((r.member_row #>> '{round_stats,goals_against}')::integer,0),
                'goal_difference',
                  (
                    coalesce(h.goals_for,0)
                    + coalesce((r.member_row #>> '{round_stats,goals_for}')::integer,0)
            ...
            r.member_row || jsonb_build_object(
              ...
              jsonb_build_object(
                ...
                'losses',
                  ...
                  + coalesce((r.member_row #>> '{round_stats,losses}')::integer,0),
                'mini_wins',
                  coalesce(h.mini_wins,0)
                  + coalesce((r.member_row #>> '{round_stats,mini_wins}')::integer,0),
                'mini_draws',
                  coalesce(h.mini_draws,0)
                  + coalesce((r.member_row #>> '{round_stats,mini_draws}')::integer,0),
                'mini_losses',
                  coalesce(h.mini_losses,0)
                  + coalesce((r.member_row #>> '{round_stats,mini_losses}')::integer,0),
                'mini_difference',
                  (
                    coalesce(h.mini_wins,0)
                    + coalesce((r.member_row #>> '{round_stats,mini_wins}')::integer,0)
    ...
    from jsonb_array_elements(
      v_result #> array['modes',v_mode,'ranking']
    ) with ordinality as r(member_row, ordinality)
    left join lateral (
      select
        sum(coalesce((lrl.standings_delta #>> '{round_stats,wins}')::integer,0))::integer
          as wins,
        sum(coalesce((lrl.standings_delta #>> '{round_stats,draws}')::integer,0))::integer
          as draws,
        sum(coalesce((lrl.standings_delta #>> '{round_stats,losses}')::integer,0))::integer
          as losses,
        sum(coalesce((lrl.standings_delta #>> '{round_stats,goals_for}')::integer,0))::integer
          as goals_for,
        sum(coalesce((lrl.standings_delta #>> '{round_stats,goals_against}')::integer,0))::integer
          as goals_against,
        sum(coalesce((lrl.standings_delta #>> '{round_stats,mini_wins}')::integer,0))::integer
          as mini_wins,
        sum(coalesce((lrl.standings_delta #>> '{round_stats,mini_draws}')::integer,0))::integer
          as mini_draws,
        sum(coalesce((lrl.standings_delta #>> '{round_stats,mini_losses}')::integer,0))::integer
          as mini_losses
      from public.league_ranking_ledger lrl
      join public.league_rounds historical_round
        on historical_round.id = lrl.league_round_id
       and historical_round.league_id = v_round.league_id
      where lrl.league_id = v_round.league_id
        and lrl.league_member_id =
          nullif(r.member_row ->> 'league_member_id','')::uuid
        and lrl.mode = v_mode
        and lrl.active = true
        and historical_round.league_round_number < v_round.league_round_number
    ) h on true;

    v_result := jsonb_set(
      v_result,
      array['modes',v_mode,'ranking'],
      v_ranking,
      false
    );
  end loop;
