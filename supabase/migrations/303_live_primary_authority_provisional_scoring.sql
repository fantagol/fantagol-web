-- ================================================================
-- FANTAGOL MIGRATION 303
-- LIVE PRIMARY AUTHORITY - PROVISIONAL SCORING RESOLUTION
-- R112-R6
--
-- Active source baseline SHA256:
-- 6B2D5269337EDCEE65414B8AFA54DA6DD7F90FE9C91075FAA1BA0A022CDAF0F4
--
-- CONTRACT:
-- * Tuttoilcalcio / degraded LIVE state may drive PROVISIONAL scoring only.
-- * LIVE score is reversible, including score decreases after VAR.
-- * END_PENDING remains provisional LIVE.
-- * public.matches remains Football-Data canonical authority.
-- * official receipts and match_result_certifications are untouched.
-- * an official certification or canonical terminal Match defeats overlay.
-- ================================================================
CREATE OR REPLACE FUNCTION public.build_points_pure_calculation_run_rpc(p_league_round_id uuid, p_surprise_candidates jsonb DEFAULT '{}'::jsonb, p_engine_version text DEFAULT 'prediction-resolution-v1'::text, p_created_by_member_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(calculation_run_id uuid, league_round_id uuid, run_version integer, calculation_status text, member_count integer, match_count integer, result_count integer, input_hash text, output_hash text)
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  loop
    for v_match in
      select
        ...
        -- R112-R6: during non-final LIVE, the selected runtime authority is
        -- the provisional scoring source. END_PENDING remains LIVE and
        -- therefore reversible; it never becomes an official final here.
        case
          when las.match_id is not null then 'live'::text
          else m.status
        end as match_status,

        case
          when las.match_id is not null then las.home_score
          else m.home_score
        end as home_score,

        case
          when las.match_id is not null then las.away_score
          else m.away_score
        end as away_score,

        -- Keep canonical Football-Data match version for certification identity.
        m.version as match_version,

        -- Deterministic LIVE input hash freezes the selected observation time.
        case
          when las.match_id is not null then las.observed_at
          else m.provider_updated_at
        end as provider_updated_at,

        m.finalised_at,

        las.authority as live_authority,
        las.source as live_authority_source,
        las.source_observation_id as live_authority_observation_id,
        las.phase as live_authority_phase,
        las.minute as live_authority_minute,
        las.version as live_authority_version
      ...
      left join lateral (
        ...
        where s.match_id = m.id
          and s.phase <> 'PRE_MATCH'
          and lower(coalesce(m.status, '')) not in ('finished', 'awarded')
          and not exists (
            select 1
            from public.match_result_certifications mrc_overlay
            where mrc_overlay.match_id = m.id
              and mrc_overlay.status = 'official'
          )
        limit 1
      ) las on true
    ...
    loop
      ...
      v_score := public.calculate_prediction_score(
        ...
        exists (
          ...
          where mrc.match_id = v_match.match_id
            and mrc.status = 'official'
            and mrc.source_match_version = v_match.match_version
        )
      );
      ...
      values (
        ...
        jsonb_build_object(
          ...
          'live_authority', v_match.live_authority,
          'live_authority_source', v_match.live_authority_source,
          'live_authority_observation_id', v_match.live_authority_observation_id,
          'live_authority_phase', v_match.live_authority_phase,
          'live_authority_minute', v_match.live_authority_minute,
          'live_authority_version', v_match.live_authority_version
        )
      );
    end loop;
  end loop;
    ...
    coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'live_authority', psrr.details ->> 'live_authority',
          'live_authority_source', psrr.details ->> 'live_authority_source',
          'live_authority_observation_id',
            psrr.details ->> 'live_authority_observation_id',
          'live_authority_phase', psrr.details ->> 'live_authority_phase',
          'live_authority_minute', psrr.details -> 'live_authority_minute',
          'live_authority_version', psrr.details -> 'live_authority_version',
...
'Builds deterministic Points Pure calculation runs. R112-R6 resolves non-final provisional match score/status from live_match_authority_states when present, while preserving public.matches + official match_result_certifications as the sole final certification authority. LIVE scores remain VAR-reversible and END_PENDING is never treated as official final.';
