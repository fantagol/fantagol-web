...
begin
  ...
  select
    ...
    + (select count(*) from public.runtime_reconciliation_profiles where enabled or auto_action_enabled)
...
begin
  ...
  if position('extensions.digest' in v_definition) = 0 then
    raise exception '076 hotfix validation failed: extensions.digest is not present';
  end if;

  if position('maintenance_policies where enabled or automatic_execution_enabled' in v_definition) > 0 then
    raise exception '076 hotfix validation failed: invalid maintenance safety columns remain';
  end if;

  if position('retention_targets where enabled or automatic_execution_enabled' in v_definition) > 0 then
    raise exception '076 hotfix validation failed: invalid retention safety columns remain';
  end if;

  if position('runtime_reconciliation_profiles where enabled or automatic_remediation_enabled' in v_definition) > 0 then
    raise exception '076 hotfix validation failed: invalid reconciliation safety column remains';
  end if;

  if position('runtime_reconciliation_profiles where enabled or auto_action_enabled' in v_definition) = 0 then
    raise exception '076 hotfix validation failed: reconciliation safety column contract not installed';
  end if;
  ...
  if exists (
    ...
    raise exception '076 hotfix validation failed: service_role has excessive 074 view privileges';
  end if;
end;
$$;
