-- ============================================================================
-- FANTAGOL
-- Migration 044: One-to-One Preview Builder
--
-- Scope:
--   - freeze One-to-One Ruleset schema v2;
--   - deterministic One-to-One Preview composition from an immutable
--     Points Preview Round Simulation;
--   - two independent 10x10 Strategy Matrices per fixture;
--   - twenty mini-challenges aggregated into one fixture result;
--   - active schedule filtering and stable payload contract;
--   - authenticated member read RPC.
--
-- Out of scope:
--   - Prediction Resolution recalculation;
--   - Fantacalcio Preview dependency;
--   - standings points and ranking ledger;
--   - realtime publication and Live State orchestration;
--   - certification commit.
--
-- Architectural rule:
--   One-to-One Preview is a parallel branch derived directly from the
--   authoritative Points Preview. It does not depend on Fantacalcio Preview
--   and stores only the new matrix/result knowledge generated by this mode.
-- ============================================================================

begin;

-- ============================================================================
-- 1. ONE-TO-ONE RULESET SCHEMA v2
-- ============================================================================

alter table public.league_scoring_profiles
  alter column one_to_one_rules set default jsonb_build_object(
    'schema_version', 2,
    'match_count', 10,
    'matrix_count', 2,
...
-- Version active profiles instead of mutating historical rules. Existing
-- Calculation Runs continue to reference their original scoring_profile_id.
with deactivated as (
  update public.league_scoring_profiles lsp
  set active = false
  where lsp.active = true
    and not (
      coalesce((lsp.one_to_one_rules ->> 'schema_version')::integer, 1) >= 2
      and coalesce((lsp.one_to_one_rules ->> 'match_count')::integer, 0) = 10
      and coalesce((lsp.one_to_one_rules ->> 'matrix_count')::integer, 0) = 2
      and coalesce((lsp.one_to_one_rules ->> 'pairings_per_matrix')::integer, 0) = 10
    )
  returning lsp.*
)
...
select
  ...
  'One-to-One Ruleset v2: two independent 10x10 matrices and twenty mini-challenges',
  true
from deactivated d;

-- ============================================================================
-- 2. OFFICIAL MATRIX PAYLOAD VALIDATOR
-- ============================================================================

create or replace function public.validate_one_to_one_strategy_payload(
  p_payload jsonb,
  p_calculation_run_id uuid,
  p_owner_member_id uuid,
  p_opponent_member_id uuid
)
returns boolean
language plpgsql
stable
...
begin
  ...
  if jsonb_array_length(p_payload -> 'pairings') <> 10 then
    return false;
  end if;

  select
    count(*)::integer,
    count(distinct (x ->> 'position')::integer)::integer,
    count(distinct (x ->> 'own_match_id')::uuid)::integer,
    count(distinct (x ->> 'opponent_match_id')::uuid)::integer
  into
    v_pairing_count,
    v_position_count,
    v_own_count,
    v_opponent_count
  from jsonb_array_elements(p_payload -> 'pairings') x
  where x ? 'position'
    and x ? 'own_match_id'
    and x ? 'opponent_match_id'
    and (x ->> 'position')::integer between 1 and 10;

  if v_pairing_count <> 10
     or v_position_count <> 10
     or v_own_count <> 10
     or v_opponent_count <> 10 then
    return false;
  end if;

  -- Every referenced match must exist for the correct member in the same
  ...
  where not exists (
          ...
          where psrr.calculation_run_id = p_calculation_run_id
            and psrr.league_member_id = p_owner_member_id
            and psrr.match_id = (x ->> 'own_match_id')::uuid
        )
     or not exists (
          select 1
          from public.prediction_score_runtime_results psrr
          where psrr.calculation_run_id = p_calculation_run_id
            and psrr.league_member_id = p_opponent_member_id
            and psrr.match_id = (x ->> 'opponent_match_id')::uuid
        );

  return v_invalid_match_count = 0;
exception
  when others then
    return false;
end;
...
-- ============================================================================
-- 3. ONE-TO-ONE PREVIEW SIMULATION BUILDER
-- ============================================================================
...
create or replace function public.get_my_one_to_one_preview_rpc(
    ...
    coalesce(
      (
        ...
        from jsonb_array_elements(
          v_simulation.digital_twin #> '{one_to_one_preview,fixtures}'
        ) x
...
revoke all on function public.validate_one_to_one_strategy_payload(
  jsonb,
  uuid,
  uuid,
  uuid
...
grant execute on function public.validate_one_to_one_strategy_payload(
  jsonb,
  uuid,
  uuid,
  uuid
...
revoke all on function public.build_one_to_one_preview_simulation_rpc(
  uuid,
  text,
  uuid,
  uuid
...
grant execute on function public.build_one_to_one_preview_simulation_rpc(
  uuid,
  text,
  uuid,
  uuid
...
revoke all on function public.get_my_one_to_one_preview_rpc(uuid)
  from public;
revoke all on function public.get_my_one_to_one_preview_rpc(uuid)
  from anon;
revoke all on function public.get_my_one_to_one_preview_rpc(uuid)
  from service_role;
grant execute on function public.get_my_one_to_one_preview_rpc(uuid)
  to authenticated;
...
comment on function public.validate_one_to_one_strategy_payload(
  jsonb,
  uuid,
  uuid,
  uuid
) is
'Validates one official One-to-One Strategy Matrix: schema v1, ten positions, ten distinct own matches, ten distinct opponent matches and authoritative runtime-result membership.';

comment on function public.build_one_to_one_preview_simulation_rpc(
  uuid,
  text,
  uuid,
  uuid
) is
'Builds an immutable parallel One-to-One Preview branch directly from a Points Preview, active schedule fixtures and two official 10x10 Strategy Matrices. It aggregates twenty mini-challenges and does not depend on Fantacalcio Preview. Service-role only.';

comment on function public.get_my_one_to_one_preview_rpc(uuid) is
'Returns the authenticated member latest One-to-One fixture preview from the parallel simulation branch, including matrix outcomes and aggregate result without duplicating authoritative prediction points.';
