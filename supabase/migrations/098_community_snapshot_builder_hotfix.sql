-- ============================================================================
-- FANTAGOL
-- Migration 098: Community Snapshot Builder Hotfix
-- Fixes PL/pgSQL output-column/CTE-column ambiguity in
...
'Community Intelligence snapshot builder. Migration 098 resolves PL/pgSQL variable/column conflicts by preferring SQL column references inside embedded statements.';
...
begin
    ...
    if v_definition is null then
        raise exception 'COMMUNITY_BUILDER_HOTFIX_FUNCTION_NOT_FOUND';
    end if;

    if position('#variable_conflict use_column' in v_definition) = 0 then
        raise exception 'COMMUNITY_BUILDER_HOTFIX_DIRECTIVE_MISSING';
    end if;
end;
$$;
