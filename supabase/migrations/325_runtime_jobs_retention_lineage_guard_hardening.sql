-- ============================================================================
-- MIGRATION 325
-- Runtime jobs retention lineage guard hardening.
--
-- Purpose:
--   - preserve all currently known live_runtime_jobs lineage references
--   - generalize the M324 parent guard from one target key to the canonical
--     live_runtime_jobs relation
--   - block retention execution when any unreceipted plan item is referenced by:
--       * live_runtime_workflow_steps.job_id
--       * live_runtime_dead_letters.job_id
--       * live_runtime_workflows.trigger_job_id
--
-- Safety:
--   - no retention target is enabled
--   - no maintenance policy is enabled
--   - no retention target definition is changed
--   - no plan/item/receipt data is changed
