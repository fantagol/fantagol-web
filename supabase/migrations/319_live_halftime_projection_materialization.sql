...
-- ============================================================================
-- FANTAGOL MIGRATION 319
-- R116-R34 LIVE HALFTIME PROJECTION MATERIALIZATION
--
-- R33 certified:
-- - primary LIVE authority correctly reaches HALFTIME;
-- - phase-change rebuild fanout succeeds;
-- - PointsPreview discarded authority phase/minute while constructing
--   digital_twin.matches, collapsing HALFTIME to generic live/TIMED;
-- - UISnapshotBuilder then faithfully inherited the degraded row.
--
-- This migration changes presentation materialization only.
-- It does NOT change scoring rules, result_phase semantics, official-final
-- authority, certification, or canonical public.matches.
-- ============================================================================
...
begin
  ...
  from (
    select
      ...
      m.kickoff,
      -- R116-R34:
      -- scoring keeps the coarse result_phase contract, but the Digital Twin
      -- match presentation must preserve the selected provisional LIVE
      -- authority phase/minute instead of falling back to canonical TIMED.
      case
        when bool_or(
          coalesce(psrr.details ->> 'live_authority_phase', '') = 'HALFTIME'
        ) then 'halftime'
        when bool_or(
          coalesce(psrr.details ->> 'live_authority_phase', '') = 'SECOND_HALF'
        ) then 'live_second_half'
        when bool_or(
          coalesce(psrr.details ->> 'live_authority_phase', '') = 'FIRST_HALF'
        ) then 'live_first_half'
        when bool_or(
          coalesce(psrr.details ->> 'live_authority_phase', '') = 'END_PENDING'
        ) then 'live_second_half'
        else min(psrr.match_status)
      end as match_status,
      min(psrr.result_phase) as result_phase,
      case
        when bool_or(
          nullif(psrr.details ->> 'live_authority_phase', '') is not null
        ) then min(
          nullif(psrr.details ->> 'live_authority_minute', '')::integer
        )
        else m.minute
      end as minute,
      case
...
'R116-R34 PointsPreview materialization preserves provisional LIVE authority phase/minute in digital_twin.matches while retaining coarse scoring result_phase and canonical final authority.';
