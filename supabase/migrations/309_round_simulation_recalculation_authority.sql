-- ============================================================================
-- FANTAGOL MIGRATION 309
-- ROUND SIMULATION RECALCULATION AUTHORITY
-- ============================================================================
-- Contract:
-- 1. An incumbent official/certified Simulation remains authoritative while a
--    recalculation candidate lineage is built.
-- 2. Recalculation candidate artifacts are preview_ready but non-publishable.
-- 3. One-to-One already follows non-publishable branch semantics and is unchanged.
-- 4. certify_round_rpc retires the previous certified Simulation atomically
--    before promoting the new UI Simulation to certified/publishable.
-- 5. Initial certification behavior remains unchanged.
-- ============================================================================
CREATE OR REPLACE FUNCTION public.build_points_preview_simulation_rpc(p_calculation_run_id uuid, p_simulation_engine_version text DEFAULT 'round-simulation-v1'::text, p_created_by_member_id uuid DEFAULT NULL::uuid, p_correlation_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(simulation_id uuid, league_round_id uuid, calculation_run_id uuid, simulation_version integer, simulation_status text, builder_status text, member_count integer, match_count integer, prediction_result_count integer, input_hash text, output_hash text, simulation_hash text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
  ...
  v_recalculation_candidate boolean := false;
begin
  ...
  into v_recalculation_candidate;
  ...
  where rs.league_round_id = v_round.id
    ...
    and not (
      v_recalculation_candidate
      and rs.status = 'certified'
      and rs.certification_id = (
        select rc.id
        from public.round_certifications rc
        where rc.league_round_id = v_round.id
          and rc.status = 'official'
  ...
  set
    ...
    preview = true,
    publishable = not v_recalculation_candidate,
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('round-simulation-version:' || v_round.id::text, 0));
...
begin
  ...
  into v_standings_preview;
    ...
    v_standings_preview :=
    public.apply_cumulative_standings_stats_internal(
      v_round.id,
      v_standings_preview
    );
...
CREATE OR REPLACE FUNCTION public.build_ui_snapshot_simulation_rpc(p_source_simulation_id uuid, p_simulation_engine_version text DEFAULT 'round-simulation-v1-ui-v1'::text, p_created_by_member_id uuid DEFAULT NULL::uuid, p_correlation_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(simulation_id uuid, source_simulation_id uuid, league_round_id uuid, calculation_run_id uuid, simulation_version integer, simulation_status text, builder_status text, match_count integer, member_count integer, mode_count integer, prediction_ui_count integer, input_hash text, output_hash text, simulation_hash text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
CREATE OR REPLACE FUNCTION public.certify_round_rpc(p_league_round_id uuid, p_calculation_run_id uuid, p_ui_simulation_id uuid, p_engine_version text DEFAULT 'round-certification-v1'::text, p_reason text DEFAULT 'automatic official round certification'::text, p_committed_by_member_id uuid DEFAULT NULL::uuid, p_correlation_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(certification_id uuid, league_round_id uuid, certification_version integer, certification_status text, calculation_run_id uuid, ui_simulation_id uuid, certification_hash text, ledger_version integer, created boolean, superseded_certification_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  if jsonb_typeof(v_standings_snapshot) <> 'object'
     ...
     or (v_standings_snapshot -> 'modes') = '{}'::jsonb then
    raise exception using
      errcode = 'P0001',
  ...
  -- Recalculation authority: keep the incumbent official Simulation intact
  -- throughout candidate construction, then retire it atomically inside the
  -- same certification transaction immediately before promoting the new UI.
  if v_previous.id is not null then
    update public.round_simulations rs
    set
      status = 'archived',
      publishable = false,
      archived_at = coalesce(rs.archived_at, v_now)
    where rs.league_round_id = v_round.id
      and rs.certification_id = v_previous.id
      and rs.status = 'certified'
      and rs.id <> v_simulation.id;
  end if;
