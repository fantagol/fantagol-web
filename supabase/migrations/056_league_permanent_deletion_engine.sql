...
-- FANTAGOL
-- Migration 056
-- League Permanent Deletion Engine
-- ============================================================
...
begin
  ...
  v_admin_member_id :=
    public.get_active_admin_member_id(p_league_id, v_user_id);
  ...
  if p_confirmation_name is null
     or btrim(p_confirmation_name) <> v_league.name then
    raise exception using
      errcode = 'P0001',
...
is
'Deletes a league and its cascade-dependent data permanently. Requires an authenticated active league admin and exact league-name confirmation.';
